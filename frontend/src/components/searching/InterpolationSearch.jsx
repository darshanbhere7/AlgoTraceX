        ...
        <div className="w-full px-4 sm:px-6 lg:px-8 pt-24 sm:pt-24 pb-8 max-w-7xl mx-auto">
            <style>{`
                ...
                @media (prefers-color-scheme: dark) {
                    ...
                    .search-element:not(.found):not(.probe):not(.low):not(.high):not(.range):hover {
            ...
            <div className="mb-6">
                ...
                <p className="text-sm text-slate-600 dark:text-white/50">
                    See how interpolation search estimates probe positions based on value distribution for faster searching.
                </p>
            </div>
                    ...
                    <>
                        <div className="flex flex-wrap gap-2 sm:gap-3 justify-center items-center min-h-[240px] sm:min-h-[280px] py-4">
                            {array.map((val, idx) => {
                                ...
                                const isProbe = idx === currentIndex;
                                const isLow = idx === lowIndex;
                                const isHigh = idx === highIndex;
                                const inRange = idx > lowIndex && idx < highIndex && lowIndex !== -1 && highIndex !== -1 && !isFound && !isProbe && !isLow && !isHigh;
                                ...
                                return (
                                    <div
                                        ...
                                        className={`search-element border px-4 py-3 min-w-[60px] text-center font-mono ${
                                            isFound ? 'found' : ''
                                        } ${isProbe ? 'probe' : ''} ${isLow ? 'low' : ''} ${isHigh ? 'high' : ''} ${inRange ? 'range' : ''} ${!searching ? 'cursor-pointer' : ''}`}
                                    >
                        ...
                        {searching && (
                            <div className="flex flex-wrap justify-center gap-4 mt-4 text-sm">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-slate-700 dark:text-white/70">Low Boundary</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-4 h-4 bg-red-500 dark:bg-red-400 rounded"></div>
                                    <span className="text-slate-700 dark:text-white/70">High Boundary</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-4 h-4 bg-purple-400 dark:bg-purple-500 rounded border border-purple-500 dark:border-purple-400"></div>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-slate-700 dark:text-white/70">Probe Position</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-4 h-4 bg-green-500 dark:bg-green-400 rounded"></div>
                                    <span className="text-slate-700 dark:text-white/70">Found</span>
                                </div>
                            </div>
                        )}
                    </>
                    ...
                    <div className="text-center">
                        {searching && probePosition !== null && (
                            <p className="text-base sm:text-lg text-slate-900 dark:text-white">
                                <span className="font-bold text-purple-600 dark:text-purple-400">Interpolating:</span> Calculated probe position at index {probePosition}
                                <br/>
                                <span className="text-sm text-slate-600 dark:text-white/60">Search range: [{lowIndex}, {highIndex}] | Checking value {array[currentIndex]}</span>
                            </p>
                        )}
                        {searching && probePosition === null && (
                            <p className="text-base sm:text-lg text-slate-900 dark:text-white">
                                ...
                                <br/>
                                <span className="text-sm text-slate-600 dark:text-white/60">Current range: [{lowIndex}, {highIndex}]</span>
                            </p>
                        )}
                ...
                <div className="bg-slate-100 dark:bg-white/[0.02] backdrop-blur-sm rounded-xl border border-slate-200 dark:border-white/[0.05] p-6 space-y-4 animate-in slide-in-from-top-2 duration-300">
                    <div>
                        ...
                        <p className="text-slate-600 dark:text-white/60 text-sm leading-relaxed">
                            ...
                            It uses the formula: <code className="bg-white dark:bg-white/10 px-1 py-0.5 rounded text-slate-900 dark:text-white">pos = low + [(target-arr[low]) × (high-low)] / (arr[high]-arr[low])</code>
                        </p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-2">
                        <div className="bg-white dark:bg-white/[0.03] rounded-lg p-4 border border-slate-200 dark:border-white/[0.05]">
                            ...
                            <p className="text-slate-500 dark:text-white/40 text-xs mt-1">O(n) worst case</p>
                        </div>
                        <div className="bg-white dark:bg-white/[0.03] rounded-lg p-4 border border-slate-200 dark:border-white/[0.05]">
                            <p className="text-slate-500 dark:text-white/40 text-xs font-medium uppercase tracking-wide mb-1">
                                Space Complexity
