        ...
        <div className="w-full px-4 sm:px-6 lg:px-8 pt-24 sm:pt-24 pb-8 max-w-7xl mx-auto">
            <style>{`
                ...
                @media (prefers-color-scheme: dark) {
                    ...
                    .search-element:not(.found):not(.jump):not(.checking):not(.block):hover {
            ...
            <div className="mb-6">
                ...
                <p className="text-sm text-slate-600 dark:text-white/50">
                    See how jump search combines jumping ahead by fixed steps with linear search in blocks.
                </p>
            </div>
                    ...
                    <>
                        <div className="flex flex-wrap gap-2 sm:gap-3 justify-center items-center min-h-[240px] sm:min-h-[280px] py-4">
                            {array.map((val, idx) => {
                                ...
                                const isJump = idx === jumpIndex;
                                const isChecking = idx === currentIndex;
                                const inBlock = idx >= blockStart && idx <= blockEnd && blockStart !== -1 && !isFound && !isJump && !isChecking;
                                ...
                                return (
                                    <div
                                        ...
                                        className={`search-element border px-4 py-3 min-w-[60px] text-center font-mono ${
                                            isFound ? 'found' : ''
                                        } ${isJump ? 'jump' : ''} ${isChecking ? 'checking' : ''} ${inBlock ? 'block' : ''} ${!searching ? 'cursor-pointer' : ''}`}
                                    >
                        ...
                        {searching && (
                            <div className="flex flex-wrap justify-center gap-4 mt-4 text-sm">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-slate-700 dark:text-white/70">Jump Position</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-4 h-4 bg-purple-400 dark:bg-purple-500 rounded border border-purple-500 dark:border-purple-400"></div>
                                    <span className="text-slate-700 dark:text-white/70">Current Block</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-4 h-4 bg-yellow-500 dark:bg-yellow-400 rounded"></div>
                                    <span className="text-slate-700 dark:text-white/70">Checking</span>
                                </div>
                                <div className="flex items-center gap-2">
                    ...
                    <div className="text-center">
                        {searching && searchPhase === "jumping" && (
                            <p className="text-base sm:text-lg text-slate-900 dark:text-white">
                                <span className="font-bold text-blue-600 dark:text-blue-400">Jumping Phase:</span> Checking block at index {jumpIndex} for <span className="font-bold">{searchValue}</span>
                            </p>
                        )}
                        {searching && searchPhase === "linear" && (
                            <p className="text-base sm:text-lg text-slate-900 dark:text-white">
                                <span className="font-bold text-purple-600 dark:text-purple-400">Linear Search Phase:</span> Searching block [{blockStart}-{blockEnd}], currently at index {currentIndex}
                            </p>
                        )}
                ...
                <div className="bg-slate-100 dark:bg-white/[0.02] backdrop-blur-sm rounded-xl border border-slate-200 dark:border-white/[0.05] p-6 space-y-4 animate-in slide-in-from-top-2 duration-300">
                    <div>
                        ...
                        <p className="text-slate-600 dark:text-white/60 text-sm leading-relaxed">
                            ...
                            It combines the benefits of linear and binary search, making it faster than linear search but simpler than binary search.
                        </p>
                    </div>
